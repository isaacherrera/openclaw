...
function collectExternalRoots(
  ...
  for (const root of roots) {
    if (!existsSync(root.sourcePath)) {
      const message = `${root.kind} path is missing: ${root.sourcePath} (${root.bindings.map((binding) => binding.configPath).join(", ")})`;
    ...
    try {
      ...
      if (!stat.isDirectory()) {
        errors.push(
          `${root.kind} path is not a directory: ${root.sourcePath} (${root.bindings.map((binding) => binding.configPath).join(", ")})`,
        );
        continue;
      }
...
export function detectHostOpenClaw(env: NodeJS.ProcessEnv = process.env): HostOpenClawState {
  ...
  if (!stateExists) {
    errorsList.push(`Resolved OpenClaw state directory does not exist: ${stateDir}`);
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    errorsList.push(`Failed to parse OpenClaw config at ${configPath}: ${msg}`);
  }

  const rootInfo = collectExternalRoots(config, stateDir);
  warningsList.push(...rootInfo.warnings);
  errorsList.push(...rootInfo.errors);
  ...
  const workspaceDir =
    ...
    typeof (((config["agents"] as Record<string, unknown>)["defaults"] as
      | Record<string, unknown>
      | undefined) ?? {})["workspace"] === "string"
      ? resolveUserPath(
          (
  ...
  if (existsSync(workspaceDir)) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      warningsList.push(`Failed to inspect workspace symlinks at ${workspaceDir}: ${msg}`);
    }
  }
...
function setConfigValue(
  ...
  const tokens = configPathStr.match(/[^.[\]]+/g);
  if (!tokens || tokens.length === 0) {
    throw new Error(`Invalid config path: ${configPathStr}`);
  }
  ...
  for (let index = 0; index < tokens.length - 1; index += 1) {
    ...
    if (!token || !nextToken) {
      throw new Error(`Invalid config path segment in ${configPathStr}`);
    }
  ...
  if (!finalToken) {
    throw new Error(`Missing final config path segment in ${configPathStr}`);
  }
