...
| **Tool deny list** | Tenant VMs include a tool deny list in `openclaw.json` (`gateway`, `sessions_spawn`, `sessions_send`) to prevent agents from modifying their own infrastructure. `cron` is allowed since v2026.3.8 (owner-only tool for scheduling). |
...
**Proactive presentation suggestion:** When a plan includes a `deep-research` step, the agent suggests adding a `presentation` step at the end (optional, user can remove). Does NOT suggest presentations for purely operational plans (demographics, places-search, enrichment) or search-only plans.
...
|-----------|----------|------------|
| `create-project` | POST /projects | Sync |
| `add-properties` | POST /projects/{id}/properties | Sync |
| `update-project` | PATCH /projects/{id} | Sync |
| `update-properties` | PATCH /projects/{id}/properties | Sync |
| `delete-properties` | DELETE /projects/{id}/properties | Sync |
| `delete-project` | DELETE /projects/{id} | Sync |
| `demographics` | POST /projects/{id}/demographics | Sync |
| `enrichment` | POST /projects/{id}/enrichment | **Async** |
| `places-search` | POST .../places/search | Sync |
| `search` | FindAll API (cobroker-search) | Async (3-7min) |
| `deep-research` | Parallel AI ultra (cobroker-deep-research) | **Async** (5-25min) |
| `presentation` | Gamma AI (cobroker-presentations) | **Async** (1-3min) |

### Proactive Presentation Suggestion

When a plan includes a `deep-research` step, the agent suggests adding a `presentation` step at the end (optional, user can remove). Does NOT suggest presentations for purely operational plans or search-only plans.

## 3-4. Plan Format & Examples

Plan is presented as a numbered list with inline keyboard buttons. Examples D-F show deep-research + presentation plan flows.
...
| 2. Customization | numCards (5), format (presentation), exportAs (**no default — must ask user**), textMode (generate), images (aiGenerated, auto-switches to web for brands) |
| 3. API Workflow | POST `/v1.0/generations` → single Node.js exec polls every 5s → extract gammaUrl + exportUrl |
| 4. Messaging | Max 4 messages: settings preview → format ask → acknowledgment → results with View Online / Download buttons |
...
**Customization:** Users can override defaults — slide count (1-60, default 5), format (presentation/document), export type (pdf/pptx — **no default, must ask user**), text mode (generate/preserve), image source (AI-generated by default; auto-switches to web images for brand/company content), text amount (brief/medium/detailed/extensive).

**Format-first UX:** Before generating, the agent shows a settings preview ("5 slides · medium text · AI images") and asks the user to pick PDF or PowerPoint. No API calls until the user taps a format button.
